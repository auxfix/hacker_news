import { getNewsFailure, getPieceOfNewsSuccess, showAlert } from 'actions';
...
export function* getNews() {
  ...
  try {
    ...
    for (const id of rundom_10_news) {
      const serverNewsItem: HackerStoryDTO = yield hacker_api.getHackerStory(id);
      const userId: string = serverNewsItem.by;
      const serverUser: UserDTO = yield hacker_api.getUser(userId);
      const internalNewsItem = HackerStory_Dto_to_Internal(serverNewsItem);
      internalNewsItem.img = rundom_images[rundom_image_indexes[newsIndex]];
      internalNewsItem.karma = serverUser.karma;
      internalNewsItem.num = newsIndex; 
      yield put(getPieceOfNewsSuccess(internalNewsItem, newsIndex));
      newsIndex++;
    }    
  } catch(error) {
    console.error(error);
